...
也可以直接告诉 AI“创建桌面入口”，由 AI 执行创建或刷新操作。
...
警情等级配置使用以下固定顺序。新增 `03 黄标+星标` 后，原有后续等级代码顺延：

| 等级代码 | 等级名称 | 颜色 | 铃声 | 图标包 |
|---|---|---|---|---|
| `00` | 普通 | `#73cb6d` | `MediumAlarm.wav` | `普通.zip` |
| `01` | 星标 | `#E56659` | `CriticalAlarm.wav` | `星标.zip` |
| `02` | 黄标 | `#eba54d` | `MediumAlarm.wav` | `黄标.zip` |
| `03` | 黄标+星标 | `#eba54d` | `MediumAlarm.wav` | `黄标.zip` |
| `04` | 预警 | `#73cb6d` | `CommonlyAlarm.wav` | `预警.zip` |
| `05` | 指令 | `#73cb6d` | `CommonlyAlarm.wav` | `指令.zip` |

配置前会查询现有警情等级。等级代码或等级名称相同即视为冲突：冲突项跳过，其他未冲突项继续新增。新增请求不自动重试；发生非冲突错误或响应结果不确定时停止后续写入。
