...
- Treat an explicit saved-token rejection as a recoverable authentication transition, not a retryable business error: clear that token and start exactly one interactive login flow automatically. On a rejected interactive login, transport failure, lookup failure, write failure, or any other error, stop and show the full credential-redacted response preview when available. Do not automatically retry the failed login or business request.
