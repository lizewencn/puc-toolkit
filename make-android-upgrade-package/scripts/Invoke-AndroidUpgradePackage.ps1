...
try {
    ...
    foreach ($name in @('apkPath','description','outputDirectory','versionCode','versionName','apkMd5','apkSize')) {
    ...
    if ($Action -eq 'Preview') {
        Write-Result ([ordered]@{status='previewed';packageName=$info.PackageName;versionCode=$info.VersionCode;versionName=$info.VersionName;apkMd5=$info.Md5;apkSize=$info.Size;description=[string]$manifest.description;force=[bool]$manifest.force;outputDirectory=[string]$manifest.outputDirectory;finalName=$finalName;results=@([ordered]@{item=$info.FileName;status='ready'})})
        exit 0
    }
    $finalPath = New-AndroidUpgradePackage -ApkInfo $info -Description ([string]$manifest.description) -Force:([bool]$manifest.force) -OutputDirectory ([string]$manifest.outputDirectory)
    ...
    Write-Result ([ordered]@{status='created';finalPath=$finalPath;packageName=$info.PackageName;versionCode=$info.VersionCode;versionName=$info.VersionName;apkMd5=$info.Md5;upgradeZipMd5=$validation.UpgradeZipMd5;outputSize=$validation.OutputSize;results=@([ordered]@{item=(Split-Path $finalPath -Leaf);status='created';path=$finalPath})})
    exit 0
} catch {
