...
Resolve these per batch instead of persisting them in `config.json`: exact environment, personnel number type, optional police-type GUID, optional exact root address-book organization name, and either an alias prefix with sequence/count or one exact personnel alias. Use the deployed frontend enum `102` for `人员`, `103` for `车`, and `104` for `应急车`. An exact personnel alias may include one exact dispatcher account to bind during creation. A prefix batch may bind a dispatcher only when its requested count is exactly `1`; reject a dispatcher binding before authentication or business lookups when the count is greater than `1`, because one dispatcher account cannot bind multiple personnel records.
...
3. In the graphical launcher, search dispatcher accounts with `account_list_request`, `querykey`, and `is_fuzzy_qry: 1`; display `<alias>(<account>)` and require one selected result. In prefix-batch mode, keep this field enabled only while the count is `1`; when the count becomes greater than `1`, clear any selected account, discard its pending lookup result, and disable the field. During an allowed single-record execution, require exactly one account-list match and use its alias to build `dispatcher_name` as `<alias>(<account>)`. Do not reject the request from an allowed account-list binding field; let the single create response determine whether the server accepts the binding.
