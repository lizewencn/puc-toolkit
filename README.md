...
`puc-config` 支持为指定环境预检并新增以下固定警情等级：

| 等级代码 | 等级名称 | 颜色 | 铃声 |
|---|---|---|---|
| `00` | 星标 | `#E56659` | `CriticalAlarm.wav` |
| `01` | 黄标 | `#eba54d` | `MediumAlarm.wav` |
| `02` | 普通 | `#73cb6d` | `MediumAlarm.wav` |
| `03` | 预警 | `#73cb6d` | `CommonlyAlarm.wav` |
| `04` | 指令 | `#73cb6d` | `CommonlyAlarm.wav` |

- 新增前执行认证预检，并用预览哈希确认目标环境和待新增项目。
- 现有等级的代码或名称与目标相同时，该项目视为冲突并跳过，其他未冲突项目继续新增。
- 新增过程中若确认代码或名称已存在，同样跳过该项目并继续后续项目。
- 其他失败或响应不确定时停止后续写入；新增请求不会自动重试。
- 冲突项不会被更新或覆盖。修改已有警情等级需要经过验证的更新接口。
