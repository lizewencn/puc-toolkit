...
if (-not $PlanOnly -and [string]::IsNullOrWhiteSpace($PersonnelTypeGuid)) { throw 'PersonnelTypeGuid is required for authenticated personnel operations.' }
...
$batchConfig = [ordered]@{
    ...
    rootOrganizationName=$RootOrganizationName; policeTypeGuid=$PersonnelTypeGuid; allowInsecureTls=[bool]$environmentConfig.allowInsecureTls
