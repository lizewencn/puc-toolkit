...
Before the first conversational or command-line operation for a Windows user, run `scripts/Set-PucConfigRoot.ps1 -Status`. If it returns `first-use-required`, show the resolved default path `Desktop\agentSkillLocalConfig\puc-config` and ask whether the user wants to keep it or choose another absolute path. Do not run environment initialization, authentication, reads, or writes until the user answers. The graphical launcher is the exception: on startup it must automatically initialize a missing, empty, malformed, non-absolute, or nonexistent configured root to the current user's default desktop path so the GUI can always open.
...
Store only `configRoot` in `%LOCALAPPDATA%\puc-config\setting.json`. Do not include this runtime file in the Skill package, so installing or updating the Skill cannot overwrite the user's path choice. Resolve configuration roots in this order: an explicit script `-ConfigRoot` temporary override, the non-empty `setting.json` value, then no implicit selection. Outside the graphical launcher, a missing file or an empty/missing `configRoot` is a first-use condition and must trigger the prompt. The launcher instead creates the default desktop directory and persists it before loading environments. Treat the directory selected by the environment toolbar `设置配置路径` button as a storage location and resolve the actual root as `<selected>\agentSkillLocalConfig\puc-config` (while accepting selection of either of those final two directories without duplicating them). When an old root exists, migrate the complete `puc-config` directory recursively, including `config.json`, `runtime.json`, reports, and every other child file or directory. Never merge into or overwrite a non-empty target root. When no old root exists, select an existing valid target root or initialize its `config.json` from the bundled template. Update `setting.json` only after the directory operation succeeds, and roll back the complete directory migration or initialization if persistence fails. After a successful change, do not show a modal success dialog; render the Chinese operation result and complete latest `configRoot` in `执行摘要`, and keep the latest path visible in the main status area.
...
- Update all installable packages from the GitHub repository as one versioned unit through the sibling `puc-toolkit-updater`: read `references/skill-update.md` and `references/launcher.md`.
